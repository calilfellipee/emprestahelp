...
import { Lock, Mail, Eye, EyeOff, DollarSign, User, Database } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        navigate('/dashboard'); // Redireciona somente no login
      } else {
        if (!formData.name.trim()) {
          toast.error('O campo Nome Completo é obrigatório para o cadastro.');
          setLoading(false);
          return;
        }
        await register(formData.name, formData.email, formData.password);
        toast.success('Conta criada com sucesso! Agora faça o login para entrar.');
        setIsLogin(true); // Muda para a tela de login após o sucesso
      }
    } catch (error: any) {
      // --- Bloco de Erro Corrigido ---
      console.error("Erro recebido do back-end:", error.response?.data);

      // Procura por erros de validação (ex: senha curta)
      if (error.response?.data?.errors && Array.isArray(error.response.data.errors)) {
        toast.error(error.response.data.errors[0].msg);
      
      // Procura por erros de negócio (ex: email já cadastrado)
      } else if (error.response?.data?.error) {
        toast.error(error.response.data.error);

      // Se não encontrar nenhum dos acima, mostra uma mensagem genérica
      } else {
